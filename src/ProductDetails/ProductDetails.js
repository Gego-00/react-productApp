...
const ProductDetails = (props) => {
    const colorOptions = props.data.colorOptions.map((item, pos) => {
        ...
        return (
            <img key={pos} className={classArr.join(' ')} src={item.imageUrl} alt={item.styleName} onClick={() => props.onColorChangeClick(pos)}/>
        )
    })
    ...
    const featureList = props.data.featureList.map((item, pos) => {
        ...
        return(
            <button onClick={() => props.onFeatureSelect(pos)} key={pos} className={classArr.join(' ')}>{item}</button>
        )
    })
